...
if (usingCapacitor) {
    ...
    BODY.messages.addEventListener('scroll', () => {
        lastIsAtBottom = isAtBottom(BODY.messages);
    });

    Keyboard.addListener('keyboardWillShow', () => {
        wasAtBottom = lastIsAtBottom;
    });

    // Quand le clavier est ouvert
    Keyboard.addListener('keyboardDidShow', () => {
        if (wasAtBottom) {
            setTimeout(() => scrollToBottom(), 50); // petit délai pour sécurité
        }
    });

    // Quand le clavier va se cacher
